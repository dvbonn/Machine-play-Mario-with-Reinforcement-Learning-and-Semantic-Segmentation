...
parser.add_argument("-lv", "--level", help="Level want to play/train", type=str, default="1-1")
...
parser.add_argument("-mn", "--model_name", help="Model want to load", type=str, default="dq2.pt")
...
class DQNAgent:
    ...
    def __init__(self, state_space, action_space, max_memory_size, batch_size, gamma, lr, 
        ...
        if args.transfer_learning is True:
            self.local_net = DQNModel(state_space, action_space)
            self.local_net.load_state_dict(torch.load(args.dir + '/' + args.model_name, map_location=torch.device(self.device))) 
            self.local_net.to(self.device)
            self.local_net.train()
            self.target_net = DQNModel(state_space, action_space)
            self.target_net.load_state_dict(torch.load(args.dir + '/' + args.model_name, map_location=torch.device(self.device))) 
            self.target_net.to(self.device)
            self.target_net.train()

        else:
            self.local_net = DQNModel(state_space, action_space).to(self.device)
            self.target_net = DQNModel(state_space, action_space).to(self.device)
    ...
    def recall(self):
        index = random.choices(range(self.num_in_queue), k=self.batch_size)
        
        STATE = self.STATE_MEM[index]
        ACTION = self.ACTION_MEM[index]
        REWARD = self.REWARD_MEM[index]
        STATE2 = self.STATE2_MEM[index]
    ...
    def act(self, state):
        ...
        action_list = self.local_net(state.to(self.device))
        action = torch.argmax(action_list).unsqueeze(0).unsqueeze(0).cpu()
    ...
    def experience_replay(self):
        ...
        STATE, ACTION, REWARD, STATE2, DONE = self.recall() #load old experience
...
def run():
    env = gym_super_mario_bros.make('SuperMarioBros-'+args.level+'-v0')
    env = make_env(env)
    observation_space = env.observation_space.shape
    action_space = env.action_space.n
    agent = DQNAgent(observation_space, action_space, 4000, 16, 0.9, 5e-5, args.max_exp_r, args.min_exp_r, 0.99)
    ...
    if args.train_mode is True:
        if args.transfer_learning is True:
            torch.save(agent.local_net.state_dict(),args.dir+ "/" + args.model_name + "tl_dq1.pt")
            torch.save(agent.target_net.state_dict(),args.dir+ "/" + args.model_name + "tl_dq2.pt")
        else:
            torch.save(agent.local_net.state_dict(),args.dir+ "/dq1.pt")
            torch.save(agent.target_net.state_dict(),args.dir+ "/dq2.pt")
    env.close()
