...
class DDQNAgent:
    ...
    def __init__(self, state_space, action_space, max_memory_size, batch_size, gamma, lr, 
        ...
        if args.transfer_learning is True:
            self.local_net = DDQNModel(state_space, action_space)
            ...
            self.target_net = DDQNModel(state_space, action_space)
            self.target_net.load_state_dict(torch.load(args.dir + '/' + args.model_name, map_location=torch.device(self.device))) 
            self.target_net.to(self.device)
            self.target_net.train()

        else:
            self.local_net = DDQNModel(state_space, action_space).to(self.device)
            self.target_net = DDQNModel(state_space, action_space).to(self.device)
    ...
    def recall(self):
        index = random.sample(range(self.num_in_queue), k=self.batch_size)
    ...
    def experience_replay(self):
        ...
        next_action = self.local_net(STATE2).argmax(1).unsqueeze(1)
        evaluate_action = self.target_net(STATE2).gather(1, next_action)
        target = REWARD + torch.mul((self.gamma * evaluate_action), 1 - DONE)
...
def play():
    ...
    for next_level in args.level:
        ...
        model = DDQNModel(input_shape, n_actions)
